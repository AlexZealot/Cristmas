...
void							WiFiManagerClass::WiFiBegin(bool useLedPin, uint8_t led_pin){
	/*Если принудительно не выставить режим работы и отключить энергосбережение - всё сломается*/
	WiFi.mode(WIFI_STA);
/*#ifdef ESP8266
...
#ifdef EEPROM_WIFI
	if ((getStatus() == WiFi_Connected) && _useOTA){
		ArduinoOTA.handle();
	}

	if (getStatus() == WiFi_NotConnected){
		server.handleClient();
...
void 	WiFiManagerClass::cleanEEPROM(){
	EEPROM.begin(EEPROM_SIZE);
	for (size_t i = 0; i < EEPROM_SIZE; i++)
		EEPROM.put(i, zeroByte);
	EEPROM.commit();
	EEPROM.end();
}
#endif